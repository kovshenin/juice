...
class Dummy():
	...
	def add_posts(self):
		# Posts
		for i in range(1, 10):
			post_title = self.lipsum(6)
			post_slug = self.slugify(post_title)
			post_excerpt = self.lipsum(20)
			post_content = ""
			for j in range(1, random.randrange(2, 8)):
				post_content += "<p>%s</p>" % self.get_paragraph()
		...
		post_title = "Orci malesuada tempor. etiam molestie consequat"
		post_slug = self.slugify(post_title)
		post_excerpt = self.lipsum(20)
		post_content = '<p>%s</p>[youtube url="http://www.youtube.com/watch?v=I6COwgigJ-g"]<p>%s</p><p>%s</p>' % (self.get_paragraph(), self.get_paragraph(), self.get_paragraph())
		p = Post(
			title=post_title,
			slug=post_slug,
			excerpt=post_excerpt,
	...
	def add_pages(self):
		...
		p = Page(
			title='About',
			slug='about',
			content='<p>%s</p><p>%s</p><p>%s</p><p>Don\'t forget to read my <a href="/about/bio/">biography</a>.</p>' % (self.get_paragraph(), self.get_paragraph(), self.get_paragraph())
		)
		p.put()
		...
		b = Page(
			title='Bio',
			slug='bio',
			content='<p>%s</p><p>%s</p><p>%s</p>' % (self.get_paragraph(), self.get_paragraph(), self.get_paragraph()),
			parent_page=p.key()
		)
		b.put()
	...
	def add_options(self):
		...
		options = {}
		options['twitter'] = Option(group=g.key(), name='Twitter', value='<a href=\"http://twitter.com/?status={{ title|urlencode }}{{ " "|urlencode }}{{ permalink_abs|urlencode }}\"><img src=\"/static/images/social/share/twitter.png\" /></a>')
		options['digg'] = Option(group=g.key(), name='Digg', value='<a href=\"http://digg.com/submit?url={{ permalink_abs|urlencode }}&amp;title={{ title|urlencode }}\"><img src=\"/static/images/social/share/digg.png\" /></a>')
		options['delicious'] = Option(group=g.key(), name='Delicious', value='<a href=\"http://www.delicious.com/save\" onclick=\"window.open(\'http://www.delicious.com/save?v=5&noui&jump=close&url={{ permalink_abs|urlencode }}&amp;title={{ title|urlencode }}\', \'delicious\',\'toolbar=no,width=550,height=550\'); return false;\"><img src=\"/static/images/social/share/delicious.png\" alt=\"Delicious\" /></a>')
		...
		options = {}
		options['twitter'] = Option(group=g.key(), name='Twitter', value='<a href=\"http://twitter.com/kovshenin\"><img src=\"/static/images/social/profiles/twitter.png\" /></a>')
		options['facebook'] = Option(group=g.key(), name='Facebook', value='<a href=\"http://facebook.com/kovshenin\"><img src=\"/static/images/social/profiles/facebook.png\" /></a>')
		options['linkedin'] = Option(group=g.key(), name='LinkedIn', value='<a href=\"http://www.linkedin.com/in/kovshenin\"><img src=\"/static/images/social/profiles/linkedin.png\" /></a>')
		options['digg'] = Option(group=g.key(), name='Digg', value='<a href=\"http://digg.com/users/kovshenin\"><img src=\"/static/images/social/profiles/digg.png\" /></a>')
		options['delicious'] = Option(group=g.key(), name='Delicious', value='<a href=\"#\"><img src=\"/static/images/social/profiles/delicious.png\" /></a>')
		options['deviantart'] = Option(group=g.key(), name='DeviantArt', value='<a href=\"#\"><img src=\"/static/images/social/profiles/deviantart.png\" /></a>')
		options['flickr'] = Option(group=g.key(), name='Flickr', value='<a href=\"#\"><img src=\"/static/images/social/profiles/flickr.png\" /></a>')
		options['google'] = Option(group=g.key(), name='Google', value='<a href=\"#\"><img src=\"/static/images/social/profiles/google.png\" /></a>')
		options['lastfm'] = Option(group=g.key(), name='Last.fm', value='<a href=\"#\"><img src=\"/static/images/social/profiles/lastfm.png\" /></a>')
		options['youtube'] = Option(group=g.key(), name='YouTube', value='<a href=\"#\"><img src=\"/static/images/social/profiles/youtube.png\" /></a>')
		options['technorati'] = Option(group=g.key(), name='Technorati', value='<a href=\"#\"><img src=\"/static/images/social/profiles/technorati.png\" /></a>')
		options['stumbleupon'] = Option(group=g.key(), name='StumpleUpon', value='<a href=\"#\"><img src=\"/static/images/social/profiles/stumbleupon.png\" /></a>')
		options['reddit'] = Option(group=g.key(), name='Reddit', value='<a href=\"#\"><img src=\"/static/images/social/profiles/reddit.png\" /></a>')
		options['myspace'] = Option(group=g.key(), name='MySpace', value='<a href=\"#\"><img src=\"/static/images/social/profiles/myspace.png\" /></a>')
	...
	def add_comments(self):
		...
		for i in range(1, 21):
			...
			comments[i] = Comment(
				name=self.lipsum(count=1),
				email=self.lipsum(count=1, make_slug=True) + '@email.com',
				url='http://' + self.lipsum(count=2, make_slug=True) + '.com',
				content=self.lipsum(count=random.randrange(10, 50)),
				object_link=p.key(),
				parent_comment=pc
			)
			comments[i].put()

	def lipsum(self, count=50, make_slug=False):
		lipsum = """
		...
		if make_slug:
			return self.slugify(output)
		else:
			return output

	def get_paragraph(self):
		return self.lipsum(count=random.randrange(random.randrange(10, 30), random.randrange(80, 180)))
